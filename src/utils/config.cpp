...
DaemonConfig load_config(const std::string& path) {
    ...
    while (std::getline(file, line)) {
        ...
        if (section == "daemon" || section.empty()) {
            ...
            else if (key == "enable_audio") config.enable_audio = (value == "true" || value == "1");
            else if (key == "audio_shm_name") config.audio_shm_name = value;
            else if (key == "audio_buffer_seconds") config.audio_buffer_seconds = std::stoul(value);
